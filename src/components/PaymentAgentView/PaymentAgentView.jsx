...
class PaymentAgentView extends React.Component {
  constructor(props) {
    ...
    window.Twilio.Flex.Actions.addListener("afterAcceptTask", (payload) => {
      this.resetPay();
      window.Twilio.Flex.Actions.invokeAction("SetComponentState", {
        name: "AgentTaskCanvasTabs",
        state: { selectedTabName: "call" },
      });
    });
    ...
    if (
      window.Twilio.Flex.CallCanvasActions.Content.fragments.length > 0 &&
      window.Twilio.Flex.CallCanvasActions.Content.fragments[0].props.children
        .key &&
      window.Twilio.Flex.CallCanvasActions.Content.fragments[0].props.children
        .key === "pay-btn"
    ) {
      console.log(
        `Found Key !!!!! ${window.Twilio.Flex.CallCanvasActions.Content.fragments[0].props.children.key} ignoring`
      );
    } else {
  ...
  displayPaymentForm = () => {
    ...
    window.Twilio.Flex.Actions.invokeAction("SetComponentState", {
      name: "AgentTaskCanvasTabs",
      state: { selectedTabName: "payment-tab" },
    });
  };
    ...
    sync
      ...
      .then((list) => {
        ...
        list
          ...
          .then((paginator) => {
            ...
            paginator.items.forEach((item) => {
              console.log(item);
              this.addEventToState(item.data);
            });
          })
        ...
        list.on("itemAdded", (event) => {
          ...
          this.addEventToState(event.item.data);
        });
      })
      .then((subscribed) => {
  ...
  latestPaymentState = () => {
    console.log("CJC: Get latest payment state", this.state.aapStatus);
